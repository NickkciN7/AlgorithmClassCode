...
const MetalBar_1 = require("../homework7/MetalBar");
...
function bestValueForFused(bagCapacity, counts, values) {
    if (values.length === 0) {
        return 0;
    }
    let metalBars = [];
    for (let i = 0; i < counts.length; i++) {
        metalBars.push(new MetalBar_1.MetalBar(counts[i], values[i]));
    }
    return bestValueForFusedHelper(bagCapacity, metalBars, 0);
}
function bestValueForFusedHelper(bagCapacity, metalBars, index) {
...
/**
 * solution similar to class solution. didn't have solution anymore, but I do have a pic of a table on classes'
 * piazza channel I represented with bestValues array and figured it out
 *
 * uncomment the console logs in this function and only execute one test case to see how the table bestValues is built
 */
function bestValueForFusedDP(bagCapacity, counts, values) {
    if (values.length === 0) {
        return 0;
    }
    let metalBars = [];
    ...
    const bestValues = Array(rowCount);
    ...
    for (let barIndex = 1; barIndex < rowCount; barIndex++) {
        ...
        if (currBar.count >= colCount) {
            bestValues[barIndex - 1].forEach((val, index) => (bestValues[barIndex][index] = val));
            //   console.log(bestValues);
        }
        for (let weight = currBar.count; weight < colCount; weight++) {
            bestValues[barIndex][weight] = Math.max(bestValues[barIndex - 1][weight], currBar.count * currBar.value +
