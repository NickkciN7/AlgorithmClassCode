...
function countFivesHelper(num, count) {
    if (num < 1) {
        return count;
    }
    return countFivesHelper(Math.floor(num / 10), num % 10 === 5 ? count + 1 : count);
}
...
function pickTreesHelper(trees, index) {
    ...
    return Math.max(trees[index] + pickTreesHelper(trees, index + 2), pickTreesHelper(trees, index + 1));
}
