 ...
 */
function mergeSort(arr: number[], recursiveLevel?: number): number[] {
  ...
  if (length < 2) {
    console.log(
      `${indentationString}base case curr arr (will be return of left/right): `,
      arr
    );
    return arr;
  }
  ...
  const left = mergeSort(
    ...
    hasRecursiveLevel ? recursiveLevel + 1 : undefined
  );
